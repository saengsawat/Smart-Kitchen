...
Milestones 0 (scaffolding + CI) and 1 (domain core + schema) are complete. ADR-001/002/003/008 are DECIDED. **Build resumed 2026-09-22** under D-021 (token darkenings adopted), D-022 (stubbed identity port; auth vendor still open) and D-023 (M3 build opens on the locked screens; hallway test deferred to pre-release). The build reference is **prototype v4** (`docs/design/mockups/smart-kitchen-prototype.html`: Menu tab replaces Recipes per D-020, Home adapts to kitchen state per OQ-D9) plus `docs/design/copy-deck.md` (binding strings) and `docs/design/tokens.md`. **Held:** Menu page internals and Home dashboard states until Dean's additional-function list is triaged and D-002 is ratified. Model routing note (PO, 2026-09-22): Opus-class tickets run on the default Opus with the architect doing a detailed acceptance pass of the code itself; Fable only for tickets the PO or architect flag as super important (cost). Live truth: [STATUS.md](STATUS.md) · [DECISIONS.md](DECISIONS.md).
