...
Milestones 0 (scaffolding + CI) and 1 (domain core + schema) are complete. ADR-002/003/008 are DECIDED. The **D-018** foundation follow-up batch (M1-E3, tickets M1-T6 → T10) is **complete (2026-09-15)**; engineering is **stopped for PO review**. Ticketed but not dispatched: M1-T11 (ledger write-path aborted-COMMIT fix) and M3-E0-T1..T3 (design completion, docs only). Visual direction v2 is adopted (**D-019**). D-002 (MVP scope cut) remains PROPOSED and gates M3 (UI); M2 (API) awaits PO go-ahead on its cost gates. The UI/UX plan exists as docs only (`docs/design/`) — nothing client-side is built. Live truth: [STATUS.md](STATUS.md) · [DECISIONS.md](DECISIONS.md).
