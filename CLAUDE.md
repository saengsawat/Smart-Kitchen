...
Milestones 0 (scaffolding + CI) and 1 (domain core + schema) are complete. ADR-002/003/008 are DECIDED. The **D-018** batch (M1-T6 → T10) and the **D-018a** queue (M1-T11, M3-E0-T1..T3) are **complete (2026-09-16)**; engineering is **stopped for the PO/Dean decision session** (`docs/po/decision-brief-2026-09.md`). Visual direction v2 is adopted (**D-019**); the M3-E0 design gate's engineering deliverables exist (14-screen prototype, `docs/design/copy-deck.md` binding for allergen/provenance strings, `docs/design/tokens.md`); its PO-run steps and D-002 still gate M3 build tickets. Nothing is dispatched. D-002 (MVP scope cut) remains PROPOSED and gates M3 (UI); M2 (API) awaits PO go-ahead on its cost gates. The UI/UX plan exists as docs only (`docs/design/`) — nothing client-side is built. Live truth: [STATUS.md](STATUS.md) · [DECISIONS.md](DECISIONS.md).
