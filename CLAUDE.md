...
Milestones 0 (scaffolding + CI) and 1 (domain core + schema) are complete; ADR-001/002/003/008 are DECIDED. Build resumed 2026-09-22 under D-021 (token darkenings), D-022 (stubbed identity port; auth vendor still open) and D-023 (M3 build on the locked screens; hallway test deferred to pre-release). **Merged since (all reviewed, squash, CI green):** M3-T1 Expo shell, M2-T1 identity port + tenancy over HTTP, M3-T2 onboarding with the allergy gate, M3-T3 inventory list + item detail, M2-T2 write endpoints + undo + dev seed, M3-T4a client plumbing to the write endpoints, M3-T4b add food (S6 to S9) with engine-generated screening fixtures, M3-T4c web target plus Expo start hygiene (PO browser walk owed; the OneDrive checkout cannot bundle for web, see STATUS). Every locked screen except Shopping (M3-T5) and Profile (M3-T6) exists on the phone against real inventory endpoints; add-food lookup and verdicts are fixture-backed until M2-T4. **Written, not dispatched:** M2-T3 (household create/join/members, item creation; Opus/Opus) held for the PO's usage reset. **Blocked on PO decisions:** D-024 (Dean's additions triage, ratifies D-002 and unblocks the Menu and Home tickets) and A3 household permissions (gates M2-T4: restrictions storage and server-side screening). Build reference: prototype v4 (`docs/design/mockups/smart-kitchen-prototype.html`), `docs/design/copy-deck.md` (binding strings; deck wins on safety copy, prototype wins on the rest), `docs/design/tokens.md`. Model routing (PO, 2026-09-22/23): default Opus for rule-23 tickets with a detailed architect pass at acceptance, Sonnet for routine work, Fable only when super important. A real-device camera pass is owed before any release. Live truth: [STATUS.md](STATUS.md) · [DECISIONS.md](DECISIONS.md).
