...
- ✅ **M3-T2 DONE 2026-09-22:** onboarding merged: S1 (fixture sign-in, create or join a household with `CHEN-482`), S2 (nine allergen chips from the taxonomy labels, Standard/Severe per selection, custom allergens, the explicit "No known allergies for {member}" option, Continue refused until every member is covered), a layout-level routing guard over every route, onboarding state behind the client port with an explicit `noneConfirmed`, household contracts with a taxonomy consistency test, and a copy scan that now covers JSX text. Sonnet/Opus, PASS after two fix rounds (the second caught a stale-read redirect). 1095 tests without a DB. Deck §11 added; deck-vs-prototype precedence rule recorded. Handoff: docs/handoff/M3-T2.{worker,review}.md. **Dean can now walk S1 → S2 → Home in Expo Go.**
- **Next:** M3-T3 inventory list + item detail (S4, S5) against the M2-T1 endpoint; M2-T2 write endpoints after it. Held: Menu internals (D-020), Home states (OQ-D9) pending Dean's list.
