...
15. **Logging redaction:** structured JSON logger (pino via Fastify) with a UUIDv7 correlation id minted per request (a caller-supplied request id is ignored) and echoed on `x-correlation-id`; framework request logging is off and the request line is built from an allowlist (method, matched route pattern, status, duration, opaque ids), never the raw URL, query or headers; a denylist formatter redacts personal keys (names, emails, tokens, profile and allergy fields) and scans string values for emails and bearer credentials, with an idempotent error reducer so a 500 keeps its type and scrubbed message (M2-T1). Value-shape scanning does not catch a name embedded in free text, so log payload review stays part of code review.
...
- **Product-health metrics** (the ones that matter — MVP_PRD §12):
  - **Inventory correction rate** (IMPLEMENTED M1-T7, definition DECIDED at acceptance 2026-09-14) — per household-item over the ledger: `statement_count` = every ledger row that is **not** a system-generated over-consumption clamp; `user_adjustment_count` = user-authored `ADJUSTMENT` rows; `clamp_count` = `OVER_CONSUMPTION`-flagged rows (ADR-008's clamp policy); `correction_event_count = user_adjustment_count + clamp_count`; `correction_rate = correction_event_count / statement_count` (undefined when there are no statements). System- or AI-confirmed adjustments count as statements, not corrections. The API's own writes feed the metric directly (M2-T2): a user `ADJUSTMENT` through `POST /v1/inventory/items/{itemId}/transactions` counts as a correction, and an over-consumed removal produces the clamp row that counts as the other kind. **Ruling (2026-09-22):** undo rows are user `ADJUSTMENT`s and count as corrections; undoing a statement is evidence the statement was wrong. Two API refusals keep the metric honest: a negative target quantity is rejected rather than planned as an overshoot, and an undo that would overshoot a lot is refused rather than clamped, so no clamp row is ever minted by a malformed or impossible request. Because every clamp sits immediately after exactly one causing statement, the rate is bounded to `[0, 1]` by construction. Items with no ledger rows produce **no row**, not a NULL-rate row — do not "fix" this by joining `inventory_items`. Read all-time via the `inventory_correction_telemetry` view (`security_invoker`, SELECT-only for `sk_app`; a dashboard role also needs SELECT on the ledger table) or bounded to a `[since, until)` window over `recorded_at` via `readCorrectionTelemetry`, which computes the same definitions in SQL over the base table (household aggregate = sum counts, then divide — never a mean of item rates). *Primary metric*: it measures whether the system's beliefs track reality, which is the hypothesis (brief §18C). A falling rate with steady usage = product working; session counts can look great while inventory rots. Weekly/rolling presentation and alerting are M9's job.
