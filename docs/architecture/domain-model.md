...
| **AllergenAssertion** | Product/ingredient contains/may-contain allergen (§3) | Kinds are `CONTAINS | MAY_CONTAIN` **only** — there is deliberately no negative/"free-from" kind; invented negative kinds are rejected and degrade to unknown (M1-T4). Absence-of-data ≠ absence-of-allergen (SR-2). **Unreadable data ≠ absence of data (M1-T6):** a present-but-malformed `allergens` field (not an array — `"peanut"`, `5`, `{}`, `null`) or a non-object array entry (`["peanut"]`, `[null]`) is recorded as uninterpreted (`UnknownReason` `MALFORMED_ALLERGEN_DATA`), which refuses the absence licence and raises `UNRECOGNIZED_ALLERGEN_DATA`, exactly as an unrecognized assertion code does; a missing field and an empty array remain honest "no assertions". *(ENGINEERING INFERENCE, M1-T6; closes the M1-T4 review's mandatory pre-M4 gate (a).)* |
...
- **OQ-1** Lot granularity for fungibles (rice, oil): per-purchase lots vs single pooled lot per item? `PROPOSED`: lots always, pooled display. Decide with real UX. *M1-T1 note: quantities derive and clamp per lot, so a pooled-lot choice changes UX, not ledger semantics.* *M1-T8 note: the selection rule now exists in the domain — `planLotConsumption(item, { qtyMicros | amount, policy })` implements **FEFO** (expiry ascending, undated last, ties by `acquiredAt` then lot order) and **FIFO** (`acquiredAt` ascending, undated last, then lot order); both are pure and take the policy as an argument, and plans produced by `planLotConsumption` are clamp-free by construction. **PROPOSED default: FEFO** — it minimises waste, which is the product's stated purpose, and it degrades gracefully to acquisition order when no lot carries an expiry. Status **PROPOSED (ENGINEERING INFERENCE)**, not DECIDED: the default is a UX decision the PO may overrule, and the domain itself has no default. Still open: what a caller does with a `shortfallMicros > 0` plan (record the overshoot and accept the ledger's clamp — the honest correction-rate signal — or refuse and prompt) — a product question for the PRD before M8 wires meal-log decrements.*
