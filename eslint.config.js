...
export default tseslint.config(
  {
    ignores: [
      ...
      "**/.expo/**",
      // M3-T4c: apps/mobile/expo-env.d.ts is generated by the Expo CLI the
      // first time `expo start`/`expo export` runs against this app (already
      // in the root .gitignore since M3-T1, so it was always expected to
      // exist locally, just never linted before). It sits outside
      // apps/mobile/tsconfig.json's "include" list by design (a triple-slash
      // reference file, not app source), which made ESLint's type-aware
      // parser (`projectService: true`) fail the whole lint run with "was
      // not found by the project service" once the file existed on disk,
      // rather than a warning on the file itself.
